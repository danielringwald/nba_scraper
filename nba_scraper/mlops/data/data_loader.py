...
class DataLoader:
    ...
    def feature_engineer_last_n_winner(self, df: pd.DataFrame, n_games: int = 5) -> pd.DataFrame:
        ...
        sub_df = df.loc[
            (df[sgc.HOME_TEAM_ABBREVIATION] == team_abbreviation) | (df[sgc.AWAY_TEAM_ABBREVIATION] == team_abbreviation)
        ...
        for i in range(n_games, len(sub_df)):
            ...
            print(current_game)
            print(last_n_games[[sgc.DATE, sgc.HOME_TEAM_ABBREVIATION, sgc.HOME_TEAM_SCORE, sgc.AWAY_TEAM_ABBREVIATION, sgc.AWAY_TEAM_SCORE]])
            ...
            row[lngf.DATE_DAY] = datetime.date.fromisoformat(str(current_game[sgc.DATE])).day
            row[lngf.DATE_MONTH] = datetime.date.fromisoformat(str(current_game[sgc.DATE])).month
            row[lngf.DATE_YEAR] = datetime.date.fromisoformat(str(current_game[sgc.DATE])).year
            ...
            print(pd.DataFrame(row, index = [team_abbreviation + str(current_game[sgc.DATE])]))
            return pd.DataFrame()
