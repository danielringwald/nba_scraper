...
class BoxScoreMapper():
    ...
    @staticmethod
    def map_df_to_box_score_game(df: pd.DataFrame) -> GameBoxScore:
        ...
        for idx, (_, row) in enumerate(df.iterrows()):  
            ...
            player_score = BoxScoreRow(
                PLAYER_NAME=row.get('Starters') or row.get('PLAYER_NAME') or "Unknown",
                MP=str(row.get('MP', '0')),
                FG=to_int(row.get('FG')),
                FGA=to_int(row.get('FGA')),
                FG_PERCENTAGE=to_float(row.get('FG%')) if pd.notnull(row.get('FG%')) else 0,
                THREE_POINTERS_MADE=to_int(row.get('3P')),
                THREE_POINTERS_ATTEMPTED=to_int(row.get('3PA')),
                THREE_POINTERS_PERCENTAGE=to_float(row.get('3P%')) if pd.notnull(row.get('3P%')) else 0,
                FT=to_int(row.get('FT')),
                FTA=to_int(row.get('FTA')),
                FT_PERCENTAGE=to_float(row.get('FT%')) if pd.notnull(row.get('FT%')) else 0,
                ...
                GAME_SCORE=to_float(row.get('GmSc')) if pd.notnull(row.get('GmSc')) else 0,
